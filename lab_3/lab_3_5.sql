...
-- 11. Составьте запросы на выборку данных с использованием ФУНКЦИИ ROLLUP.
SELECT GENRE.NAME AS GENRE_NAME,
       COUNT(*)
FROM MOVIE
         JOIN MOVIE_GENRE ON MOVIE.ID = MOVIE_GENRE.MOVIE_ID
         JOIN GENRE ON MOVIE_GENRE.GENRE_ID = GENRE.ID
GROUP BY ROLLUP (GENRE.NAME);

-- 12. Составьте запросы на выборку данных с использованием
-- ОПЕРАТОРА MERGE ЯЗЫКА МАНИПУЛИРОВАНИЯ ДАННЫМИ.
MERGE INTO MOVIE LEFT_MOVIE
USING (SELECT MOVIE.ID
       FROM MOVIE
                JOIN PRODUCER ON MOVIE.PRODUCER_ID = PRODUCER.ID
       WHERE PRODUCER.NAME = 'Энтони Руссо') RIGHT_MOVIE
ON (LEFT_MOVIE.ID = RIGHT_MOVIE.ID)
WHEN MATCHED THEN
    UPDATE
    SET LEFT_MOVIE.PRODUCER_ID = 1;